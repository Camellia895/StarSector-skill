...
| `verify_uitags_fix.js` | red | base | ★**批量 uiTags 改动的三合一复核**：对每个 mod 比对"备份快照 vs 当前"——**目标格差异数 > 0**、**EOL 字节级一致**、**结构一致**（列数/行数），并检查所有新标签都在核心中文词表（可用 `--ext-map` 扩充）内；最后对全库复扫 `check_uitags_zh.js`，按 mod 列出剩余命中并标注"核心英文残留 有/无"。备份布局兼容 `<mod>/hull_mods.csv`（标准位置）与 `<mod>/bypath/<相对路径>__hull_mods.csv`（非标准位置，如 `data\config\hull_mods.csv`）。2026-09-18 实测 20 个 mod 全 OK | `node verify_uitags_fix.js <backupDir> <modRoot...> [--ext-map=<json>]` |
| `audit_uitags_visibility.js` | — | — | **逐行可见性审计**（回答"这个英文标签玩家看不看得见"）：列出仍有非中文 uiTags 的每一行，带 `hidden`/`hiddenEverywhere` 判定（★玩家可见 / 不可见(hidden)）与 `name`/`desc` 摘要。用途：决定该标签是否值得汉化、以及**作者自定标签**（`DEVTOOL`/`Unique`/`Utility`）的处置依据。2026-09-18 实测：`EptaConsortium` 的 10 处 `DEVTOOL` 全是 `hidden + DEBUG: You should never see this in game` | `node audit_uitags_visibility.js <modsDir> [--core=<核心data目录>]` |
| `uitags_lib.js` | — | — | 共用小库：`readCsvNoComments()`（**跳过 `#` 注释行/模板行**——引擎会忽略而 `csvlib` 会解析成数据，实测造成 TreasureHunt/Trails 的假命中，见 `iron-rules.md` R21）与 `isCommentRecord()` | `require('./uitags_lib.js')` |
