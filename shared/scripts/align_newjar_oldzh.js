// Class-level alignment: NEW jar classes vs OLD translated jar classes (same class names).
// Produce newEn -> oldZh pairs valid for the NEW jar, using LCS pairing.
const fs = require('fs');
const path = require('path');
const { walkCp, decodeModifiedUtf8 } = require('./patcher.js');

const NEW_DIR = process.argv[2];
const OLDZH_DIR = process.argv[3];
...
function lcsPairing(a, b) {
  ...
  const segB = [];
  let prev = -1;
  for (const [ai, bj] of anchors) { segB.push([prev + 1, bj]); prev = bj; }
  segB.push([prev + 1, m]);
...
const oldZhMap = new Map();
for (const f of walk(OLDZH_DIR)) oldZhMap.set(path.relative(OLDZH_DIR, f).replace(/\\/g, '/'), litList(fs.readFileSync(f)));

const mapping = new Map();
const byClass = new Map();
let total = 0, classes = 0;
for (const f of walk(NEW_DIR)) {
  const rel = path.relative(NEW_DIR, f).replace(/\\/g, '/');
  if (!oldZhMap.has(rel)) continue;
  const n = litList(fs.readFileSync(f));
  const z = oldZhMap.get(rel);
  const pairs = lcsPairing(n, z);
  let cls = 0;
  const clsPairs = [];
  for (const [ni, zi] of pairs) {
    const en = n[ni], zh = z[zi];
...
console.log('classes aligned:', classes, 'unique pairs:', mapping.size);
