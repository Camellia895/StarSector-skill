...
## 3.5 安装顺序事故（2026-09 The Vass，勿重蹈）

- 数据层注入若需要"从备份还原目录再重跑"（rm -rf mods/<Mod> + cp），会把**已安装的补丁 jar 一并回滚**。
- 本会话实测：补丁 jar 安装后被数据层重跑覆盖回英文版，靠 `check_jar_stragglers.js`（A=307）才暴露。
- **固定顺序：数据层全部定稿 → 还原/重跑 → 最后装 jar → 立刻跑 G4 闸门（含 SHA-256 比对）**。
