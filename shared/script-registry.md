...
| 脚本 | 用途 | 闸门 |
|---|---|---|
| `verify_identifiers.js` | 全量 class 的 `NameAndType`/`Class` 名称不得含 CJK | G4 |
| `check_u0001.js` | 映射层：含 `\u0001` 的键与其译文数量一致 | G4 |
| `verify_u0001_jar.js` | 补丁后 jar 中每个含 `\u0001` 常量与原始 jar 数量一致 | G4 |
| `scan_stragglers.js` | 补丁后英文 UI 残留扫描（排除 Intrinsics/SMAP/调试日志） | G4 |
| `sweep_sentences.js` | **句子级**复查：专治注释夹折叠漏译、弯引号键不匹配 | G4 |
| `verify_patched.js` | 补丁目录综合：`\u0001` + 英文句子残留（迁移场景） | G4 |
| `verify_all_data.js` | **数据层全量复查**：LunaSettings Text/Header/Radio、variants `displayName`、faction 舰队/官职名、`designTypeColors` 键唯一且与 CSV 匹配、`custom_entities`、`customStarts` | G3 |
| `check_csv_quotes.js` | 弯引号计数 + 归一化后行列数预检（铁律 R1） | G3 |
| `check_rules_arg_quotes.js` | rules script 列命令参数内嵌引号（铁律 R2，**不崩溃只截断**） | G3 |
| `check_font_glyphs.js` | 中文字库字形覆盖（铁律 R3，缺字形显示 `?`） | G3 |
| `LoadTest.java` | 离线类加载/实例化 + `hull_mods.csv`/`*.system` 脚本类存在性（**需 `-noverify` 与 logs 路径属性**，见 `env.md` §4） | G5 |
| `check_refs.js` | 引用完整性：variant/`.ship`/`.skin`/`default_ship_roles.json` → hull/武器/hullmod/wing；`weapon_data.csv` ↔ `.wpn` ↔ `.proj` | G5 |
| `check_assets.js` | 全部 `graphics/…` 字面量是否存在（**必须同时搜 mod + core + 其它已装 mod**，跳过注释行） | G5 |
| `check_sprites.js` | 源码 `getSprite("分类","键")` 是否在 mod 或 core 的 `settings.json` 有定义 | G5 |
| `check_deprecated.js` | mod 是否用了 0.98a API 的 `@Deprecated` 成员 | G5 |
| `cmp_strings.js` | 新旧 jar 字符串常量对比（判源码/jar 漂移；用"原 jar 每条常量是否作为子串出现在新 jar 常量集合里"，不要求精确相等） | G5 |
| `csvcheck.js` | CSV 表头/列数/指定列取值（**完整状态机**，处理引号内换行） | 排查 |
| `jsonkeys.js` | 容错 JSON 顶层键对比 | 排查 |
| `JsonProbe.java` | 用**游戏自带 `org.json`** 验证数据宽松语法（铁律 R8 的权威工具） | G3/排查 |
...
| 脚本 | 归属 skill | 用途 |
|---|---|---|
| `check_content.js` | `starsector-mod-localization-content` | 译文内容自检（占位符/`%%`/`${}`/长度/空译文） |
